...
typedef struct prov_ctx_st {
    const OSSL_CORE_HANDLE *handle;  // ядро OpenSSL
    OSSL_LIB_CTX *libctx;        //  онтекст библиотеки
    BIO_METHOD *corebiometh;     // BIO-методы (если нужны свои методы типа static int core_bio_write(BIO *bio, const char *buf, int len))
    //CRYPTO_RWLOCK* lock;             // Ѕлокировка (опционально)
} PROV_CTX;
